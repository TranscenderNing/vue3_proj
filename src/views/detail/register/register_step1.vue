<template>
  <div class="wrap">
    ...
    <div class="center">
        ...
        <div class="container">
            <div class="item" @click="changeTime(item)" v-for="item in workData.bookingScheduleList" :key="item" :class="{active: item.status === -1 || item.availableNumber === -1, active1: item.workDate === workTime.workDate}">
                <div class="c-top">{{ item.workDate }}-{{ item.dayOfWeek}}</div>
                <div class="c-footer">
                    <div v-if="item.status === -1">停止挂号</div>
                    <div v-if="item.status === 0">
                        {{ item.availableNumber === -1? '约满了':`有号${item.availableNumber}个` }}
                    </div>
...
const getWorkData = async (page = 1)=>{
    pageNo.value = page
    let  res = await reqHospitalWork(pageNo.value, limit.value, $route.query.hoscode as string, $route.query.depcode as string)
    console.log(res)
    if(res.code === 200){
...
const getDoctorWorkData = async ()=>{
    let  res = await reqHospitalDoctor($route.query.hoscode as string, $route.query.depcode as string,workTime.value.workDate)
    if(res.code === 200){
...
let  morningArr = computed(()=>{
    return docArr.value.filter(item=>{
...
const goStep2 = (doctor:any)=>{
    $router.push({path: '/detail/register_step2',query:{docId: doctor.id}})
}
